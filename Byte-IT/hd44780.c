...
void lcd_goto(uint8_t pos)
  {
	if ( pos < 17 ) { // 16x1 LCD: column < 9
						// 16x2 LCD: column < 17
      lcd_command(0x80 + pos);
   } else {
      lcd_command(0xB8 + pos - 9);	// 16x1 LCD: column - 1
											// 16x2 LCD: column - 9
   }
  }
...
Returns:  none
*************************************************************************/
void lcd_puts(unsigned char column, char *string)
  {
//	lcd_clrscr();
  ...
  void lcd_putsMax(unsigned char column, char *string, unsigned char max)
  {
	lcd_clrscr();
    unsigned char c = column;
	unsigned char i = 0;
